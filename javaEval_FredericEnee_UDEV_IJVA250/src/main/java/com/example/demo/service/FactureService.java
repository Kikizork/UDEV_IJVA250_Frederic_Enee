...
public class FactureService {
	...
	public List<FactureDto> findAllFactures() {
		...
		return factureRepository.findAll().stream().map(f -> factureDto(clientDto(f.getClient()), f)).collect(toList());
	}
	...
	private ClientDto clientDto(Client client) {
		return new ClientDto(client.getId(), client.getNom(), client.getPrenom(),
				getAgeClient(client.getDateNaissance()), client.getDateNaissance());
	}

	private FactureDto factureDto(ClientDto clientDto, Facture f) {
		List<LigneFactureDto> ligneFactureDtos = f.getLigneFactures().stream().map(lf -> ligneFactureDto(lf))
				.collect(toList());
	...
	private LigneFactureDto ligneFactureDto(LigneFacture lf) {
		...
		return new LigneFactureDto(new ArticleDto(article.getId(), article.getLibelle(), article.getPrix()),
				lf.getQuantite());
	}
